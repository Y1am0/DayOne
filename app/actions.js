...
import {
    ...
    deleteHabitWithName,
    getHabits,
    getHabitByNameAndUserId,
    getHabitProgress,
    getLogs,
    getUserHabitLogs,
} from "@/lib/dbHabits";
import { revalidatePath } from "next/cache";
import { calculateDisciplineDays } from "@/utils/calcualateDisciplineDays";
        ...
        tools: {
                        ...
                        return (
                            <div className="space-y-4">
                                {habits.map((habit) => (
                                    <div
                                        key={habit.id}
                                        className="p-4 space-y-2 border rounded shadow-sm"
                                    >
            ...
            addHabit: {
                ...
                parameters: z.object({
                    ...
                    difficulty: z
                        .string()
                        .describe(
                            "The difficulty of the habit. Can be 'TYPICAL', 'HARD', or 'VERY_HARD'"
                        ),
                    color: z
                        .string()
                        .describe(
                            "The color of the habit. Can be one of those hex colors:  #ef4444, #22c55e, #3b82f6, #eab308, #f97316, #a855f7, #64748b"
                        ),
                }),
                generate: async function* ({ title, difficulty, color }) {
                    ...
                    const added = `Habit added: ${newHabit.title} (Difficulty: ${newHabit.difficulty}, Color: ${newHabit.color})`;
                    revalidatePath("/");
                    return added;
                },
            },
            deleteHabit: {
                ...
                parameters: z.object({
                    name: z
                        .string()
                        .describe(
                            "The name of the habit to delete. If the user input doesn't match the habit name exactly, run a fuzzy search to find the closest match."
                        ),
                }),
            ...
            viewHabitProgress: {
                ...
                generate: async function* ({ name }) {
                    ...
                    const habit = await getHabitByNameAndUserId(
                        name,
                        userSession.user.id
                    );
                    console.log(habit);
            ...
            predictDisciplineLikelihood: {
                description: "Predict the likelihood of staying disciplined based on historical data",
                parameters: z.object({
                    name: z.string().describe("The name of the habit"),
                }),
                generate: async function* ({ name }) {
                    const userSession = await authenticate();
                    yield "Analyzing your historical data...";
                    const habit = await getHabitByNameAndUserId(name, userSession.user.id);
                    if (!habit) {
                        return `Habit with name "${name}" not found.`;
                    }
                    const likelihood = calculateDisciplineDays(habit.dailyLogs);
                    return `Based on your historical data for the habit "${name}", your likelihood of staying disciplined is ${likelihood.toFixed(2)}%.`;
                },
            },
